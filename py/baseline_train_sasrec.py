...
def run_a_trail(train_config,log_file=None, save_mode=False,save_file=None,need_train=True,warm_or_cold=None):
    ...
    sasrec_config={
        ...
        "hidden_units": int(train_config['embedding_size']),
        "num_blocks": 2,
        "num_heads": 1,
        "dropout_rate": 0.2,
        "l2_emb": 1e-4,
    ...
    model = SASRec(sasrec_config).npu()
    ...
    if not need_train:
        ...
        for batch_id,batch_data in enumerate(valid_data_loader):
            batch_data = [x_.npu() for x_ in batch_data]
        ...
        for batch_id,batch_data in enumerate(test_data_loader):
            batch_data = [x_.npu() for x_ in batch_data]
            ui_matching = model.forward_eval(batch_data[0].long().npu(),batch_data[1].long().npu(),batch_data[2].long().npu())
            pre.extend(ui_matching.detach().cpu().numpy())
            label.extend(batch_data[-1].cpu().numpy())
            users.extend(batch_data[0].cpu().numpy())
...
if __name__=='__main__':
    ...
    # embedding_size_ = [32, 64, 128, 156, 512]
    embedding_size_ = [256]
    save_path = "/root/CoLLM-main/CoLLM/minigpt4rec-log/"
    # try:
    #     f = open("rec_mf_search_lr"+str(lr_[0])+".log",'rw+')
    # except:
    #     f = open("rec_mf_search_lr"+str(lr_[0])+".log",'w+')
    ...
    for lr in lr_:
        for wd in dw_:
            for embedding_size in embedding_size_:
                ...
                save_file = save_path + f"sports_sasrec_best_d{embedding_size}.pth"
                run_a_trail(train_config=train_config, log_file=f, save_mode=True, save_file=save_file)
    if f is not None:
        f.close()
