...
try:
    ...
    dist_utils = types.SimpleNamespace(
        get_world_size=lambda: dist.get_world_size() if _is_ddp_init() else 1,
        get_rank=lambda: dist.get_rank() if _is_ddp_init() else 0,
        is_dist_avail_and_initialized=lambda: _is_ddp_init(),
    )
try:
