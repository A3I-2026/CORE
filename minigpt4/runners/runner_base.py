...
try:
    ...
    def get_world_size():
        return dist.get_world_size() if _ddp_ready() else 1
    def get_rank():
        return dist.get_rank() if _ddp_ready() else 0
    def is_main_process():
        return get_rank() == 0
...
class RunnerBase:
    ...
    def _load_checkpoint(self, url_or_filename):
        ...
        # [修复] 检查 resume_reset_epoch 标志
        resume_reset_epoch = False
        if hasattr(self.config.run_cfg, "resume_reset_epoch"):
            resume_reset_epoch = self.config.run_cfg.resume_reset_epoch
        elif isinstance(self.config.run_cfg, dict):
            resume_reset_epoch = self.config.run_cfg.get("resume_reset_epoch", False)

        if not resume_reset_epoch:
            ...
            self.start_epoch = 0
            logging.info("Loaded weights from {}, but SKIPPED optimizer loading (Stage Transition).".format(url_or_filename))
